...
def test_sf_rate(SFgambit, dai, nft, SFsunft, creator, owner, queen, stream_rate, seven_days):

    SFgambit.deposit(SFsunft, stream_rate, {"from": owner})

    chain.sleep(seven_days)
    SFgambit.tryUnlock(SFsunft, {"from": owner})

    # Owner should now have 2 NFTs from unlocking
    assert nft.balanceOf(owner) == 1

    # Principle should be 100 times seven_days
    assert SFgambit.getPrincipal(SFsunft) == stream_rate*seven_days
